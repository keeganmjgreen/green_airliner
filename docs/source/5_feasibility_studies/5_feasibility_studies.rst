...
- The thick colored lines show the SoC of the airliner decreasing between airports and the SoC increasing rapidly again when the airliner flies over the intermediate airports and is refueled in mid-air.
...
.. image:: lon-lat-soc-1-jfk-range.png

If the destination airport is not within this range (as is expectedly the case), any airport within the range can and must be selected as an intermediate airport over which to fly and be refuelled. While flying in the completely opposite direction of the destination airport is obviously undesirable and may filter out a number of candidates, it is not in general obvious which candidate of intermediate airport would be best and by what metric. In general, some questions arise regarding the selection of the intermediate airport:

1. Should the intermediate airport be the one closest to the destination airport?
2. Should the intermediate airport be the one closest to the direct flight path that a jet-fueled airliner would have taken to the destination airport?

We will refer to these as criterion 1 and 2. While it may appear that the airport selected by criterion 1 would necessarily be the same as that selected by criterion 2, and vice versa, this is in general not the case. In an extreme and illustrative example, an intermediate airport "a" could be 2000 km in the very rough direction of the destination airport, while another intermediate airport "b" could be a mere 100 km away and extremely close to a direct flight path to the destination. Neglecting for now the inefficiency of refueling at airport "b" so close to the origin airport, the intermediate airports "a" and "b" would be the obvious choices by criteria 1 and 2 respectively---but airports "a" and "b" are strikingly different.

In any case, if PIT is selected as the intermediate airport to fly over, then the airliner can be refueled and we repeat the process by again modelling a linear decrease in SoC as the airliner flies from PIT in a yet-undecided direction:

.. image:: lon-lat-soc-2-pit-range.png

DEN is selected as the next intermediate airport. The process can be repeated until the destination airport (LAX international airport) is within range of the airliner (with some reserve fuel), as is now the case in flying from DEN to LAX.

.. image:: lon-lat-soc-3-den-range.png
