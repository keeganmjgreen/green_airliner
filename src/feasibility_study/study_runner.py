...
from src.feasibility_study.study_params import DISTANCE_KM_LOOKUP, MJ_PER_GJ, BaseAirliner, Uav
...
def run_study(
    ...
    uav: Uav,
    waypoints: List[str],
    n_refuels_by_waypoint: Optional[Dict[str, int]] = None,
) -> pd.DataFrame:
    ser = pd.DataFrame(
        columns=[
            "index",
    ...
    ser.loc[
        (len(ser), airliner.time_into_flight_h, waypoints[0])
    ...
    for i in range(len(waypoints) - 1):
        ...
        ser.loc[
            (len(ser), airliner.time_into_flight_h, waypoints[i + 1])
        ] = airliner.energy_quantity_MJ
        ...
        if i < len(waypoints) - 2:
            if n_refuels_by_waypoint is not None:
